...
// API Base URL - Environment variable use kora hoyeche
...
export default function AdminRoute() {
  ...
  const fetchAllData = async () => {
    ...
    try {
      ...
      const [resB, resS, resP, resSold, resN] = await Promise.all([
        fetch(`${API_BASE_URL}/api/admin/buyers`, { headers }),
        fetch(`${API_BASE_URL}/api/admin/sellers`, { headers }),
        fetch(`${API_BASE_URL}/api/admin/products`, { headers }),
        fetch(`${API_BASE_URL}/api/admin/sold-items`, { headers }),
  ...
  const handleStatusChange = async (id, newStatus) => {
    const token = localStorage.getItem("adminToken");
    try {
      const res = await fetch(`${API_BASE_URL}/api/admin/products/${id}/status`, {
        method: "PATCH",
        headers: { 
  ...
  const handleRemove = async (type, id) => {
    ...
    // API Route Mapping (Backend routes er sathe mil rekhe)
    const endpoint = 
      type === 'product' ? `products/${id}` : 
      type === 'notification' ? `notifications/${id}` : 
      type === 'buyer' ? `buyers/${id}` : `sellers/${id}`;
        ...
        {activeSection === 'notifications' && (
          <div className="space-y-6">
            <div className="bg-blue-800 text-white p-8 rounded-lg shadow-lg">
              ...
              <p className="text-lime-300 text-sm mt-1">Direct inquiries sent by sellers.</p>
            </div>
            <div className="grid grid-cols-1 gap-4">
              {notifications.length === 0 ? (
                <div className="bg-white/50 p-10 rounded-xl text-center text-gray-500 border-2 border-dashed border-gray-300">No requests found.</div>
              ) : (
                notifications.map((note) => (
                  <div key={note.id} className="bg-white p-6 rounded-xl shadow-md border-l-8 border-blue-800 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 hover:shadow-lg transition-shadow">
                    <div className="flex-1">
                      ...
                      <p className="text-[10px] text-gray-400 mt-2">Date: {new Date(note.createdAt).toLocaleString()}</p>
                    </div>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-lg shadow-lg p-6">
              <div className="relative mb-6">
                ...
                <input type="text" placeholder="Search Buyers..." value={buyerSearch} onChange={(e) => setBuyerSearch(e.target.value)} className="w-full pl-12 pr-4 py-3 border border-gray-200 rounded-lg outline-none focus:border-blue-500" />
              </div>
        ...
        {activeSection === 'sellers' && (
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-lg shadow-lg p-6">
              <div className="relative mb-6">
                <FaSearch className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400" />
                <input type="text" placeholder="Search Sellers..." value={sellerSearch} onChange={(e) => setSellerSearch(e.target.value)} className="w-full pl-12 pr-4 py-3 border border-gray-200 rounded-lg outline-none focus:border-blue-500" />
              </div>
              <div className="overflow-x-auto">
        ...
        {activeSection === 'products' && (
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-lg shadow-lg p-6">
              <div className="relative mb-6">
                <FaSearch className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400" />
                <input type="text" placeholder="Search Products..." value={productSearch} onChange={(e) => setProductSearch(e.target.value)} className="w-full pl-12 pr-4 py-3 border border-gray-200 rounded-lg outline-none focus:border-blue-500" />
              </div>
              <div className="overflow-x-auto">
                <table className="w-full text-left">
                  <thead className="bg-blue-800 text-lime-300">
                    <tr><th className="px-6 py-4">Product</th><th className="px-6 py-4">Price</th><th className="px-6 py-4">Status</th><th className="px-6 py-4">Seller</th><th className="px-6 py-4">Action</th></tr>
                  </thead>
                  <tbody>
                      ...
                      <tr key={p.id} className="border-b hover:bg-blue-50">
                        ...
                        <td className="px-6 py-4">
                          {p.status?.toLowerCase() === "sold" ? (
                            <span className="bg-red-100 text-red-600 px-3 py-1 rounded-full text-xs font-bold border border-red-200 uppercase">Sold</span>
                          ) : (
                            <select value={p.status || "active"} onChange={(e) => handleStatusChange(p.id, e.target.value)} className="border rounded px-2 py-1 text-sm bg-gray-50 outline-none focus:ring-2 focus:ring-blue-400">
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-lg shadow-lg p-6">
              <div className="overflow-x-auto">
                <table className="w-full text-left">
                  <thead className="bg-blue-800 text-lime-300">
                    <tr><th className="px-6 py-4">Product</th><th className="px-6 py-4">Price</th><th className="px-6 py-4">Buyer</th><th className="px-6 py-4">Email</th><th className="px-6 py-4">Date</th></tr>
                  </thead>
                  <tbody>
                    {soldHistory.map((item) => (
                      <tr key={item.id} className="border-b hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4">{item.order?.buyer?.name || "N/A"}</td>
                        <td className="px-6 py-4"><span className="text-xs bg-blue-50 text-blue-700 px-2 py-1 rounded">{item.order?.buyer?.email}</span></td>
                        <td className="px-6 py-4 text-sm text-gray-500">{new Date(item.order?.createdAt).toLocaleDateString()}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
