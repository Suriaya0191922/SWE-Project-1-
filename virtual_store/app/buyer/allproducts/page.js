...
function ProductsListContent() {
  ...
  const fetchProducts = async () => {
    try {
      ...
      if (response.ok) {
        ...
        // UPDATE: Amra ekhon filter korbo na, shob product set korbo
        setProducts(data);
      }
    } catch (error) {
  ...
  const addToCart = async (product) => {
    // Safety check: Logic-e abar check korchi status
    if (product.status?.toLowerCase() === "sold" || product.status?.toLowerCase() === "pending") {
      return;
    }
    ...
    <div style={styles.container}>
      <div style={styles.header}>
        <div>
          ...
          <p style={styles.subtitle}>
            Showing {filteredProducts.length} items from CUET Campus
          </p>
        </div>
      </div>
      ...
      {filteredProducts.length === 0 ? (
        <div style={styles.emptyState}>
          ...
          <p style={styles.emptyText}>Try searching with different keywords</p>
        </div>
      ) : (
        <div style={styles.grid}>
          {filteredProducts.map((p) => {
            // Normalized status to handle lowercase/uppercase
            const currentStatus = p.status?.toLowerCase() || "active";
            const isSold = currentStatus === "sold";
            const isPending = currentStatus === "pending";
            const canPurchase = !isSold && !isPending;
              ...
              <div key={p.id} style={styles.card}>
                <div style={styles.imageWrapper}>
                  <img 
                    ...
                    alt={p.productName} 
                    style={{ ...styles.image, opacity: canPurchase ? 1 : 0.6 }} 
                    onError={(e) => e.target.src = "/placeholder.jpg"}
                  />
                  {isSold && <div style={styles.soldOverlay}>SOLD OUT</div>}
                  {isPending && <div style={styles.pendingOverlay}>UNDER REVIEW</div>}
                </div>
                ...
                <div style={styles.cardContent}>
                  ...
                  <div style={styles.priceRow}>
                    ...
                    <span style={{ 
                      ...styles.statusBadge, 
                      ...(styles[currentStatus] || styles.active) 
                    }}>
                      {p.status || "Active"}
                    </span>
                  </div>
                  ...
                  <div style={styles.actions}>
                    <button
                      style={{ 
                        ...styles.cartBtn, 
                        ...(!canPurchase ? styles.disabledBtn : {}) 
                      }}
                      onClick={() => canPurchase && addToCart(p)}
                      disabled={!canPurchase}
                    >
                      {isSold ? "❌ Out of Stock" : isPending ? "⏳ Pending Approval" : "🛒 Add to Cart"}
                    </button>
...
const styles = {
  ...
  soldOverlay: { position: "absolute", top: "50%", left: "50%", transform: "translate(-50%, -50%)", backgroundColor: "rgba(239, 68, 68, 0.8)", color: "white", padding: "8px 16px", borderRadius: "8px", fontWeight: "bold", pointerEvents: "none" },
  pendingOverlay: { position: "absolute", top: "50%", left: "50%", transform: "translate(-50%, -50%)", backgroundColor: "rgba(245, 158, 11, 0.8)", color: "white", padding: "8px 16px", borderRadius: "8px", fontWeight: "bold", pointerEvents: "none" },
